...
export class ChatService {
     ...
     */
    async visionCompletions(messages, options = {}) {
        const visionConfig = configurationService.getVisionConfig();
        const modelName = this.environmentService.getModelName('vision');
        const requestBody = {
            model: modelName,
            messages,
            thinking: { type: 'enabled' },
            stream: false,
            temperature: options.temperature ?? visionConfig.temperature,
            top_p: options.topP ?? visionConfig.topP,
            ...(options.maxTokens && { max_tokens: options.maxTokens })
        };
        console.info('Request ZAI chat completions API for vision analysis', { model: modelName, messageCount: messages.length });
        try {
